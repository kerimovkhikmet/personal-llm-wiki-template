...
- A digest of a long video (talk, lecture, panel) produced with Gemini - see `_gemini-youtube-digest-prompt.md` and the section below.
...
## Video digests (Gemini)

Gemini turns a long video into a compact, timestamped digest, so the transcript (often 50-100k tokens for a multi-hour video) never enters the repo. Run `_gemini-youtube-digest-prompt.md` - paste the video URL and the prompt into Gemini in one message, one-shot for videos of about 2 hours or less and a three-pass chapter protocol for longer ones - and save the output here:

- Filename: `YYYY-MM-DD-digest-<slug>.md`, one note per video; long videos use `## Chapter - HH:MM` sections so each chapter can be ingested and cited independently.
- Keep the provenance frontmatter the prompt emits: `source` (canonical URL), `tool: gemini`, `duration`, `date`, `tags: [digest]`.
